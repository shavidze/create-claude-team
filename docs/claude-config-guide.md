...
8-საფეხურიანი vertical slice gate-ებით: PM-spec (დამტკიცება) → plan-to-docs →
architect+designer (API კონტრაქტი frontend/backend-ის გაყრამდე) → backend →
frontend → qa → review (+security თუ auth/გადახდები/PII) → ფინალური `/check` +
`git log` ვერიფიკაცია. Engineering Manager-ი (Claude) ყოველ საფეხურს ამოწმებს
და ჩავარდნილ gate-ზე ჩერდება. გამოიყენება მხოლოდ ნამდვილად მრავალროლიან
სამუშაოზე — multi-agent ~15× ტოკენი ღირს single-agent-თან შედარებით.
